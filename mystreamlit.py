...
def user_input_features():
    ...
    trad_job = {
        'admin.': 'Administrativo', 'blue-collar': 'Obrero',
        'entrepreneur': 'Emprendedor', 'housemaid': 'Empleada del hogar',
        'management': 'Gerente', 'retired': 'Jubilado',
        'self-employed': 'Autónomo', 'services': 'Servicios',
        'student': 'Estudiante', 'technician': 'Técnico',
        'unemployed': 'Desempleado', 'unknown': 'Desconocido'
    }
    trad_marital = {
        'divorced': 'Divorciado', 'married': 'Casado',
        'single': 'Soltero', 'unknown': 'Desconocido'
    }
    trad_edu = {
        'primary': 'Primaria', 'secondary': 'Secundaria',
        'tertiary': 'Universitaria', 'unknown': 'Desconocido'
    }
    trad_contact = {
        'cellular': 'Móvil', 'telephone': 'Teléfono fijo',
        'unknown': 'Desconocido'
    }
    trad_month = {
    ...
    trad_poutcome = {
        'failure': 'Fracaso', 'other': 'Otro',
        'success': 'Éxito', 'unknown': 'Desconocido'
    }
    ...
    # Preprocesado manual de pdays — igual que en el EDA
    if pdays_raw == -1:
        pdays = 'no_contact'
    elif pdays_raw <= 200:
        pdays = 'recent'
...
st.write("Perfil del cliente configurado (nota: 'pdays' ha sido categorizado internamente para el modelo):")
...
if st.button("Predecir Suscripción de Depósito"):
    ...
    prob_yes = probabilidades[1]  # índice 1 = clase 'yes'
    prob_no  = probabilidades[0]  # índice 0 = clase 'no'

    st.subheader("Resultado de la Predicción")

    if prediccion == 'yes':
        st.success(f"**El modelo predice: SÍ contratará el depósito.** (Probabilidad: {prob_yes*100:.2f}%)")
    else:
        st.warning(f"**El modelo predice: NO contratará el depósito.** (Probabilidad: {prob_no*100:.2f}%)")

    # Barra de probabilidad visual
    st.write("**Probabilidad de suscripción (Sí):**")
    st.progress(float(prob_yes))
    st.caption(f"{prob_yes*100:.2f}% de probabilidad de contratar el depósito")

    # Tabla de probabilidades
    st.write("**Probabilidades por clase:**")
    prob_df = pd.DataFrame({
        'Clase': ['No contrata (no)', 'Sí contrata (yes)'],
        'Probabilidad': [f"{prob_no*100:.2f}%", f"{prob_yes*100:.2f}%"]
    })
    st.table(prob_df)